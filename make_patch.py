...
if __name__ == '__main__':
    ...
    data_list = os.listdir(data_path)[:6]
    data_list.sort()
    train_test_idx = np.random.choice((0, 1), len(data_list), p=(.5, .5))
    ...
    move_data(data_path, test_list[:max(2, int(len(test_list) * .2))], draw_path)
